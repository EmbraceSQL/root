...
export namespace Api {
  export function parseEchoTypeArray(from: string | null) {
    ...
    return rawArray.map((e: unknown) => {
      // eslint-disable-next-line @typescript-eslint/restrict-template-expressions
      return parseEchoType(`${e}`);
    });
  }

  export function parseEchoTypeNestedArray(from: string | null) {
    if (from === null) return null;
    const rawArray = JSON.parse(from);
    return rawArray.map((e: unknown) => {
      // eslint-disable-next-line @typescript-eslint/restrict-template-expressions
      return parseEchoTypeNested(`${e}`);
    });
  }

  export function parseEchoType(from: string | null) {
    return from;
  }

  export function parseEchoTypeNested(from: string | null) {
    return from;
  }
}
// end string parsers
