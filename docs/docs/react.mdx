...
```shell
mkdir -p ./src/server
npx @embracesql/cli generate express --database postgres://postgres:postgres@localhost/dvdrental > ./src/server/dvdrental.ts
```

And update the express server entry point:

<CodeBlock language="typescript" title="./src/server/main.ts">{ExpressSource}</CodeBlock>
...
```shell
mkdir -p src/client
npx @embracesql/cli generate react --database postgres://postgres:postgres@localhost/dvdrental > ./src/client/dvdrental-react.ts
```
...
<CodeBlock language="html" title="index.html">{HtmlSource}</CodeBlock>

And attach a react component to `#root`.

<CodeBlock language="typescript" title="./src/client/main.tsx">{ReactSource}</CodeBlock>

And just a tiny bit of CSS:

<CodeBlock language="css" title="./src/client/main.css">{CssSource}</CodeBlock>
